 ...
 */
export async function fetchText(url, params) {
    const fullUrl = params
        ? `${url}?${new URLSearchParams(Object.entries(params).map(([k, v]) => [k, String(v)])).toString()}`
        : url;
    const controller = new AbortController();
...
export function handleError(e) {
    if (e instanceof HttpError) {
        if (e.status === 404)
            return JSON.stringify({ error: "Resource not found. Please verify your query." });
        if (e.status === 429)
            return JSON.stringify({ error: "Rate limit exceeded. Please wait before retrying." });
        if (e.status === 503)
            return JSON.stringify({ error: "Service temporarily unavailable. Try again shortly." });
