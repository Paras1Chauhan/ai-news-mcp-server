...
export async function getHackerNewsStories(params) {
    try {
        const topIds = await fetchJson(`${HACKERNEWS_API_URL}/topstories.json`);
        const fetchLimit = Math.min(100, topIds.length);
        // Fetch stories concurrently
        const results = await Promise.allSettled(topIds.slice(0, fetchLimit).map((id) => fetchJson(`${HACKERNEWS_API_URL}/item/${id}.json`)));
        const stories = [];
        for (const result of results) {
            ...
            if (params.filter_ai) {
                ...
                if (!isAiRelated(text, AI_KEYWORDS))
                    continue;
            }
            stories.push({
                id: item.id,
            ...
            if (stories.length >= params.max_results)
                break;
        }
...
//# sourceMappingURL=hackernew.js.map