...
import { ArxivSearchSchema, HackerNewsSchema, DevToSchema, RssFeedSchema, TrendingSummarySchema, } from "./schemas.js";
...
// ─────────────────────────────────────────────
server.registerTool("ai_news_get_arxiv_papers", {
...
// ─────────────────────────────────────────────
server.registerTool("ai_news_get_hackernews", {
...
// ─────────────────────────────────────────────
server.registerTool("ai_news_get_devto_articles", {
...
// ─────────────────────────────────────────────
server.registerTool("ai_news_get_rss_feed", {
...
// ─────────────────────────────────────────────
server.registerTool("ai_news_get_trending_summary", {
...
Returns JSON dashboard with: { generated_at, summary: { papers?, hackernews?, news? } }`,
    ...
    try {
        ...
        if (params.include_hn) {
            tasks.push(getHackerNewsStories({
                max_results: params.max_items_each,
                filter_ai: true,
            }).then((r) => { summary.hackernews = JSON.parse(r); }));
        }
        if (params.include_news) {
            tasks.push(getRssFeed({
                source: "all",
                max_results: params.max_items_each,
