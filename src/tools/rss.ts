...
import type { RssFeedInput } from "../schemas.js";
...
async function fetchRssSource(sourceKey: string, maxResults: number): Promise<NewsItem[]> {
...
export async function getRssFeed(params: RssFeedInput): Promise<string> {
  try {
    ...
    if (params.source === "all") {
      ...
      const results = await Promise.all(
        Object.keys(RSS_SOURCES).map((key) => fetchRssSource(key, perSource))
      );
